...
module.exports.getFileURL = function getFileURL(ctx) {
    return new Promise((res, rej) => {
        ...
        if (!photo) {
            ...
            return getFileLink(ctx, file.file_id)
                .then((response) => {
		...
		*/
            if (channel.guild.me.hasPermission("MANAGE_WEBHOOKS") === false) {
                ...
                channel
                    .send(attachment)
                    .then((e) => {
                        let embed = new MessageEmbed().setAuthor(ctx.message.from.first_name, e.attachments.first().proxyURL).setDescription(ctx.message.text).setColor("RANDOM");
                        channel
                            .send(embed)
                            .then(() => e.delete())
                            .catch((error) => {
                ...
                channel
                    .fetchWebhooks(require("fs").readFileSync("WebhookID").toString())
                    .then((webhooks) => {
                        let webhook = webhooks.first();
                        if (!webhook || !webhook.send)
                            channel.createWebhook("Telegram User").then((createdWebhook) => {
